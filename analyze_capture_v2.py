...
def parse_pcap_classic(path):
    """Classic libpcap format (what USBPcapCMD.exe -o writes directly,
    as opposed to the pcapng format Wireshark saves by default)."""
    ...
    if magic == 0xA1B2C3D4:
        ...
        raise ValueError(f"Not a recognized pcap/pcapng file (magic={magic:#x})")

    _ver_maj, _ver_min, _thiszone, _sigfigs, snaplen, linktype = \
        struct.unpack_from(endian + "HHiIII", data, 4)
    interfaces = [{"linktype": linktype, "snaplen": snaplen}]
    ...
    while pos + 16 <= n:
        ...
        pos += incl_len
        us = ts_frac // 1000 if nano else ts_frac
        ts = ts_sec * 1_000_000 + us
        packets.append({
            "iface_id": 0, "ts": ts, "cap_len": incl_len,
            "orig_len": orig_len, "data": pkt_data,
        })
...
def parse_capture_file(path):
    """Auto-detect pcapng vs classic pcap and parse accordingly. Both
    return the same (interfaces, packets) shape, with packet['ts'] always
    in microseconds since the Unix epoch, so everything downstream (the
    marker-window logic, timestamps) works unchanged either way."""
    with open(path, "rb") as f:
        magic = f.read(4)
...
def main():
    ...
    interfaces, packets = parse_capture_file(pcap_path)
    parsed = []
